...
async function apiLogin(email, password, signal) {
  const res = await fetch(`${API_URL}/login`, {
    ...
    // SIN credentials: 'include' para evitar CORS estricto
    signal,
  });
  ...
  if (!res.ok || data?.ok === false) {
    const err = new Error(
      data?.error ||
      data?.message ||
      (res.status === 401 ? "Credenciales inválidas." : "No se pudo iniciar sesión.")
    );
...
function attachLoginHandler() {
  ...
  // Mensaje si vienes de registro exitoso (?registered=1)
